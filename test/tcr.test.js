...
contract('Tcr', async function (accounts) {
    ...
    before(async () => {
        ...
        const twaraddress = await tcrInstance.twar();
        twarInstance = await twar.at(twaraddress);
    });
    ...
    it("working", async function () {
        ...
        let cardid = await tcrInstance.propose.call(100, "Card3", "Im3", 300, 300, 100, {from: accounts[0]}) ;
        tcrInstance.propose(100, "Card3", "Im3", 300, 300, 100, {from: accounts[0]}) ;
        console.log(cardid);
        await increaseTime(60);
        console.log("Time passed");
        console.log(await tcrInstance.getAllListings());
        await tokenInstance.transfer(accounts[1],1000);
        await tokenInstance.approve(tcrInstance.address, 500, {
            from: accounts[1]
        });
        let id = await tcrInstance.purchase.call(cardid, 500, {from: accounts[1]});
        await tcrInstance.purchase(cardid, 500, {from: accounts[1]});
        
        // tcrInstance.purchase(cardid, 500, {from: accounts[1]});
        const myCards = await twarInstance.getMyCards.call({from: accounts[1]});
