...
contract Tcr {
    ...
    function getListingDetails(bytes32 _listingHash) public view returns (bool, address, uint, uint) {
        ...
        return (listingIns.whitelisted, listingIns.owner, listingIns.deposit, listingIns.challengeId);
    }
    ...
    function propose(uint _amount, string calldata _name, string calldata _image, uint _attack, uint _defence, uint _price) external {
        ...
        // listing.data = _data;
        listingNames.push(string(abi.encode(_name, _attack, _defence, _price)));
        listing.arrIndex = listingNames.length - 1;

        Card storage card = cards[_listingHash];
        card.name = _name;
        card.image = _image;
        card.attack = _attack;
        card.defence = _defence;
        card.price = _price;
